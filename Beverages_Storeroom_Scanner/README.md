...
3. **Trigger a deployment** – every push to the default branch will build the Next.js app. The resulting URLs are:
   ...
   - `https://<vercel-domain>/Beverages_Storeroom_Scanner` → Supabase-authenticated transfer UI.
...
- `src/app/Beverages_Storeroom_Scanner/route.ts` – Serves the HTML/JS portal that talks to Supabase directly with the anon key.
