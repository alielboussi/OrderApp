...
class SupabaseProvider(context: Context, private val config: SupabaseConfig) {
    companion object {
    ...
    val supabaseUrl: String = config.resolvedSupabaseUrl
    val supabaseAnonKey: String = config.supabaseAnonKey
    ...
    @Serializable
    data class WhoamiRolesResponse(
        val outlets: List<OutletRoleInfoLite> = emptyList()
    )

    @Serializable
    data class WhoamiOutletRow(
        @SerialName("outlet_id") val outletId: String,
        @SerialName("outlet_name") val outletName: String
    )
    ...
    suspend fun listWhoamiOutlets(jwt: String): List<OutletRoleInfoLite> {
        val rolesText = http.post("$supabaseUrl/rest/v1/rpc/whoami_roles") {
            header("apikey", supabaseAnonKey)
            header(HttpHeaders.Authorization, "Bearer $jwt")
            contentType(ContentType.Application.Json)
        ...
        val rolesList = runCatching {
            relaxedJson.decodeFromString(ListSerializer(WhoamiRolesResponse.serializer()), rolesText.trim())
        }.getOrElse { emptyList() }
        ...
        val whoList = runCatching {
            relaxedJson.decodeFromString(ListSerializer(WhoamiOutletRow.serializer()), whoText.trim())
        }.getOrElse { emptyList() }
    ...
    suspend fun listWarehousesByOutlet(jwt: String, outletId: String?): List<Warehouse> {
        val url = buildString {
            append(supabaseUrl)
            append("/rest/v1/warehouses")
            append("?select=id,name,code,active")
    ...
    suspend fun listWarehouseBalanceItems(
        ...
        val kindsFilter = kinds.joinToString(",") { it }
        val encodedSearch = search?.trim()?.takeIf { it.isNotEmpty() }?.let {
        ...
        val url = buildString {
            ...
            if (encodedSearch != null) {
                append("&item_name=ilike.")
                append(encodedSearch)
            }
            if (baseOnly) {
                append("&or=(variant_key.eq.base,variant_key.is.null)")
            }
            append("&order=item_name.asc&order=variant_key.asc")
        }
        val resp = http.get(url) {
            header("apikey", supabaseAnonKey)
            header(HttpHeaders.Authorization, "Bearer $jwt")
    ...
    suspend fun listCatalogItemsMeta(jwt: String, ids: List<String>): List<CatalogItemLite> {
