...
exports.normalizeUomCode = normalizeUomCode;
...
const CODE_BY_KEY = new Map();
function formatUomLabel(unit) {
    ...
    if (lower === "straw")
        return "Straw(s)";
    if (lower === "toilet paper")
        return "Toilet Paper(s)";
    if (lower === "case")
        return "Case(s)";
    if (lower === "crate")
        return "Crate(s)";
...
function registerAlias(alias, code) {
...
function registerCode(code) {
    ...
    if (esMatch)
        registerAlias(esMatch[1], canonical);
    const sMatch = label.match(/^(.+)\(s\)$/i);
    if (sMatch)
        registerAlias(sMatch[1], canonical);
}
for (const value of DEFAULT_UOM_VALUES)
    registerCode(value);
registerCode("each");
registerAlias("pieces", "pc");
registerAlias("piece", "pc");
registerAlias("pcs", "pc");
function normalizeUomCode(value, fallback = "pc") {
...
//# sourceMappingURL=uom-codes.js.map