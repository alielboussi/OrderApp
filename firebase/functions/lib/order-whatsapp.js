...
exports.getProductEmoji = getProductEmoji;
exports.formatAcceptedOrderWhatsAppMessage = formatAcceptedOrderWhatsAppMessage;
...
function getUomEmoji(uom) {
    const normalized = String(uom ?? "").trim().toLowerCase();
    if (!normalized)
        return "🔢";
    if (normalized.includes("tray"))
        return "🍱";
    if (normalized.includes("case") || normalized.includes("crate") || normalized.includes("carton"))
        return "📦";
    ...
    if (normalized.includes("kg") || normalized.includes("kilo"))
        return "⚖️";
    if (normalized.includes("litre") || normalized.includes("liter") || normalized.includes("ltr"))
        return "🧴";
    return "🔢";
}
...
function resolveVariantDisplayLabel(baseName, fullName) {
    ...
    if (!base)
        return full;
    if (full.toLowerCase() === base.toLowerCase())
        return full;
    const dashPrefix = `${base} - `;
    if (full.startsWith(dashPrefix))
        return full.slice(dashPrefix.length).trim();
...
function groupWhatsAppItems(items) {
    ...
    for (const item of items) {
        ...
        if (baseProductName)
            group.productName = baseProductName;
        group.lines.push({
            label,
    ...
    return [...groups.values()]
        ...
        return {
            ...group,
            productName,
            lines: [...group.lines].sort((left, right) => left.label.localeCompare(right.label, undefined, { sensitivity: "base" })),
        };
    })
        .sort((left, right) => left.productName.localeCompare(right.productName, undefined, { sensitivity: "base" }));
}
function formatWhatsAppLineItem(line) {
...
function formatWhatsAppProductGroup(group) {
...
function formatAcceptedOrderWhatsAppMessage(input) {
    ...
    const total = input.items.reduce((sum, item) => sum + Number(item.qty ?? 0) * Number(item.cost ?? 0), 0);
