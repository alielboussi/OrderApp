...
exports.completeDamageReport = exports.dispatchDamageReport = exports.getDamageReportSignatureUrl = exports.getDamageReportPhotoUrl = exports.reviewDamageReport = exports.submitDamageReport = void 0;
const https_1 = require("firebase-functions/v2/https");
const firestore_1 = require("firebase-admin/firestore");
const damage_whatsapp_1 = require("./damage-whatsapp");
...
function isBranchUser(profile) {
    return profile.roles.includes("branch");
}
function normalizePhotoBase64(input) {
    const trimmed = String(input ?? "").trim();
    if (!trimmed) {
        throw new https_1.HttpsError("invalid-argument", "photoBase64 is required.");
    }
    const dataUrlMatch = /^data:image\/[a-zA-Z0-9.+-]+;base64,(.+)$/i.exec(trimmed);
    return (dataUrlMatch?.[1] ?? trimmed).replace(/\s/g, "");
}
function parsePhotoBase64(input) {
    const cleaned = normalizePhotoBase64(input);
    const buffer = Buffer.from(cleaned, "base64");
    if (!buffer.length) {
        throw new https_1.HttpsError("invalid-argument", "photoBase64 is invalid.");
    }
    return buffer;
}
function formatDamageReportNumber(outletName, sequence) {
    const prefix = outletName.trim().toUpperCase().replace(/[^A-Z0-9]/g, "") || "OUTLET";
    return `DMG-${prefix}-${String(sequence).padStart(8, "0")}`;
}
async function persistDamagePhoto(outletId, reportId, photoBase64) {
...
function normalizeSignatureBase64(input) {
    const trimmed = String(input ?? "").trim();
    if (!trimmed) {
        throw new https_1.HttpsError("invalid-argument", "signatureBase64 is required.");
    }
    const dataUrlMatch = /^data:image\/[a-zA-Z0-9.+-]+;base64,(.+)$/i.exec(trimmed);
    return (dataUrlMatch?.[1] ?? trimmed).replace(/\s/g, "");
}
function parseSignatureBase64(input) {
    ...
    const buffer = Buffer.from(cleaned, "base64");
    if (!buffer.length) {
        throw new https_1.HttpsError("invalid-argument", "signatureBase64 is invalid.");
    }
    return buffer;
}
async function persistDamageSignature(outletId, reportId, role, signatureBase64) {
...
exports.submitDamageReport = (0, https_1.onCall)({ region: "africa-south1" }, async (request) => {
    if (!request.auth?.uid) {
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    }
    const profile = await requireAppUser(request.auth.uid);
    if (!isBranchUser(profile) && !isSupervisor(profile)) {
        throw new https_1.HttpsError("permission-denied", "Outlet access required.");
    }
    ...
    if (!outletId || outletId !== profile.outletId) {
        throw new https_1.HttpsError("permission-denied", "You can only report damages for your outlet.");
    }
    if (!reportedByName) {
        throw new https_1.HttpsError("invalid-argument", "reportedByName is required.");
    }
    if (!photoBase64) {
        throw new https_1.HttpsError("invalid-argument", "A damage photo is required.");
    }
    if (!Array.isArray(lines) || lines.length === 0) {
        throw new https_1.HttpsError("invalid-argument", "At least one damaged product line is required.");
    }
    const normalizedLines = lines.map((line, index) => {
        const qty = Math.floor(Number(line.qty ?? 0));
        const name = String(line.name ?? "").trim();
        if (!name || qty <= 0) {
            throw new https_1.HttpsError("invalid-argument", `Line ${index + 1} requires a product name and qty.`);
        }
        return {
    ...
    const reportRef = db.collection(schema_1.COLLECTIONS.outletDamageReports).doc();
    const counterRef = db.collection(schema_1.COLLECTIONS.outletDamageCounters).doc(outletId);
...
async function resolveDamagePhotoUrlForWhatsApp(outletId, reportId, report) {
    ...
    await (0, firestore_1.getFirestore)()
        .collection(schema_1.COLLECTIONS.outletDamageReports)
        .doc(reportId)
...
async function sendApprovedDamageWhatsAppNotification(reportId, report, lines, supervisorName, reviewedAt) {
    const message = (0, damage_whatsapp_1.formatApprovedDamageWhatsAppMessage)({
    ...
    const photoUrl = await resolveDamagePhotoUrlForWhatsApp(String(report.outletId ?? ""), reportId, report);
    if (photoUrl) {
        await (0, wasender_1.sendWasenderGroupMessageWithImage)(message, photoUrl);
    }
    else {
        await (0, wasender_1.sendWasenderGroupMessage)(message);
    }
}
exports.reviewDamageReport = (0, https_1.onCall)({ region: "africa-south1", secrets: [wasender_1.wasenderApiKey, wasender_1.wasenderGroupId] }, async (request) => {
    if (!request.auth?.uid) {
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    }
    const profile = await requireAppUser(request.auth.uid);
    ...
    if (decision !== "accept" && decision !== "decline") {
        throw new https_1.HttpsError("invalid-argument", "decision must be accept or decline.");
    }
    const db = (0, firestore_1.getFirestore)();
    const reportRef = db.collection(schema_1.COLLECTIONS.outletDamageReports).doc(reportId);
    ...
    const reviewResult = await db.runTransaction(async (tx) => {
        ...
        if (!reportSnap.exists) {
            throw new https_1.HttpsError("not-found", "Damage report not found.");
        }
        const current = reportSnap.data();
        const currentStatus = String(current.status ?? "");
        if (currentStatus !== "awaiting_supervisor_approval") {
            throw new https_1.HttpsError("failed-precondition", "This damage report has already been reviewed.");
        }
        if (decision === "accept" && String(current.whatsappNotifyStatus ?? "") === "sent") {
            throw new https_1.HttpsError("failed-precondition", "This damage report has already been approved and notified.");
        }
        tx.set(reportRef, {
            status: nextStatus,
...
exports.getDamageReportPhotoUrl = (0, https_1.onCall)({ region: "africa-south1" }, async (request) => {
    if (!request.auth?.uid) {
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    }
    const profile = await requireAppUser(request.auth.uid);
    ...
    const db = (0, firestore_1.getFirestore)();
    const reportSnap = await db.collection(schema_1.COLLECTIONS.outletDamageReports).doc(reportId).get();
    if (!reportSnap.exists) {
        throw new https_1.HttpsError("not-found", "Damage report not found.");
    }
    ...
    if (outletId !== profile.outletId && !isSupervisor(profile)) {
        throw new https_1.HttpsError("permission-denied", "Not allowed to view this damage report.");
    }
    const inline = String(report.photoData ?? "").trim();
    if (inline) {
        return {
    ...
    const photoPath = String(report.photoPath ?? "").trim();
    if (!photoPath) {
        throw new https_1.HttpsError("not-found", "Damage photo not available.");
    }
    const bucket = (0, storage_bucket_1.getAppStorageBucket)();
    const [signedUrl] = await bucket.file(photoPath).getSignedUrl({
        action: "read",
...
exports.getDamageReportSignatureUrl = (0, https_1.onCall)({ region: "africa-south1" }, async (request) => {
    if (!request.auth?.uid) {
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    }
    const profile = await requireAppUser(request.auth.uid);
    ...
    if (role !== "driver" && role !== "offloader") {
        throw new https_1.HttpsError("invalid-argument", "role must be driver or offloader.");
    }
    const db = (0, firestore_1.getFirestore)();
    const reportSnap = await db.collection(schema_1.COLLECTIONS.outletDamageReports).doc(reportId).get();
    if (!reportSnap.exists) {
    ...
    const signaturePath = role === "driver"
    ...
    if (!signaturePath) {
        throw new https_1.HttpsError("not-found", "Signature not available.");
    }
    const bucket = (0, storage_bucket_1.getAppStorageBucket)();
...
exports.dispatchDamageReport = (0, https_1.onCall)({ region: "africa-south1" }, async (request) => {
    if (!request.auth?.uid) {
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    }
    const profile = await requireAppUser(request.auth.uid);
    ...
    if (!reportId || !driverName || !driverSignatureBase64) {
        throw new https_1.HttpsError("invalid-argument", "reportId, driverName, and driverSignatureBase64 are required.");
    }
    const db = (0, firestore_1.getFirestore)();
    const reportRef = db.collection(schema_1.COLLECTIONS.outletDamageReports).doc(reportId);
    ...
    if (report.status !== "accepted") {
        throw new https_1.HttpsError("failed-precondition", "Only accepted damage reports can be dispatched.");
    }
    const outletId = String(report.outletId ?? "").trim();
    if (!outletId) {
        throw new https_1.HttpsError("failed-precondition", "Damage report outlet is missing.");
    }
    const now = new Date().toISOString();
...
exports.completeDamageReport = (0, https_1.onCall)({ region: "africa-south1" }, async (request) => {
    if (!request.auth?.uid) {
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    }
    const profile = await requireAppUser(request.auth.uid);
    ...
    if (!reportId || !offloaderName || !offloaderSignatureBase64) {
        throw new https_1.HttpsError("invalid-argument", "reportId, offloaderName, and offloaderSignatureBase64 are required.");
    }
    const db = (0, firestore_1.getFirestore)();
    const reportRef = db.collection(schema_1.COLLECTIONS.outletDamageReports).doc(reportId);
    const reportSnap = await reportRef.get();
    ...
    if (report.outletId !== profile.outletId) {
        throw new https_1.HttpsError("permission-denied", "You can only complete damage reports for your outlet.");
    }
    if (report.status !== "loaded") {
        throw new https_1.HttpsError("failed-precondition", "Damage report must be loaded before completion.");
    }
    const outletId = String(report.outletId ?? "").trim();
    const now = new Date().toISOString();
