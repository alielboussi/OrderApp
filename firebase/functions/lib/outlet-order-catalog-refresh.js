...
const order_companion_products_1 = require("./order-companion-products");
...
function readOrderFields(source) {
    ...
    const ordersAppDisplayOrder = (0, orders_app_display_order_1.readOrdersAppDisplayOrderFromRow)(source);
    return {
        ordersAppUom,
...
async function materializeOutletOrderCatalog(db, outletId, allowlistRows) {
    ...
    const allowlistedProductIds = [
        ...new Set(allowlistRows
            .filter((row) => !row.variant_id)
            .map((row) => row.item_id)
            .filter((itemId) => itemId.length > 0)),
    ];
    const existingProductIds = new Set(catalogDocs.map((entry) => String(entry.data.productId ?? "").trim()).filter(Boolean));
    for (const companionId of (0, order_companion_products_1.getCompanionProductIdsForAllowlistedSources)(allowlistedProductIds)) {
        if (existingProductIds.has(companionId))
            continue;
