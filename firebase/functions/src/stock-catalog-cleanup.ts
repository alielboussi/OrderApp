...
type StockApiCatalogResponse = {
  generatedAt?: string;
  products?: Array<{ uuid?: string | null }>;
  warehouses?: Array<{ uuid?: string | null; name?: string | null; active?: boolean | null }>;
};
...
type CleanupPlan = {
  ...
  items_to_delete: Array<{ id: string }>;
  variants_to_delete: Array<{ id: string }>;
};
...
async function planCleanup(
...
): Promise<{ plan: CleanupPlan; deleted_docs: number }> {
  ...
  const itemsToDelete: Array<{ id: string }> = [];
...
export async function deleteCatalogRowsMissingFromApi(
  catalogPayload: StockApiCatalogResponse,
): Promise<{
  deleted_docs: number;
