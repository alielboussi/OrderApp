...
@Composable
fun OutletDamagesScreen(
    root: RootViewModel,
    onBack: () -> Unit
) {
    val session by root.session.collectAsState()
    val scope = rememberCoroutineScope()
    val logger = rememberScreenLogger("OutletDamages")
    var loading by remember { mutableStateOf(true) }
    var error by remember { mutableStateOf<String?>(null) }
    var rows by remember { mutableStateOf(listOf<SupabaseProvider.OutletWarehouseDamageDto>()) }
    ...
    if (!hasAccess) {
        AccessDeniedCard(
            ...
            message = "Damages are shown for your outlet warehouses only.",
    ...
    fun reload() {
        ...
        scope.launch {
            ...
            try {
                ...
                rows = root.supabaseProvider.fetchOutletWarehouseDamages(s.token, warehouseIds, limit = 100)
    ...
    Scaffold(
        ...
        Column(Modifier.padding(padding).padding(16.dp)) {
            ...
            when {
                ...
                rows.isEmpty() -> Text("No outlet damages recorded yet.")
                else -> LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    items(rows, key = { it.id }) { row ->
                        Card(Modifier.fillMaxWidth()) {
                            Column(Modifier.padding(12.dp)) {
                                Text(
                                    warehouseNames[row.warehouseId] ?: "Outlet warehouse",
