...
export default function CatalogMenuPage() {
    ...
    try {
      if (deleteTarget.kind === "item") {
        ...
        await logWarehouseAction({
          action: WAREHOUSE_AUDIT_ACTIONS.DELETE_PRODUCT,
          page: "/Warehouse_Backoffice/catalog/menu",
          method: "DELETE",
          entity_type: "product",
          entity_id: deleteTarget.item.id,
          entity_name: deleteTarget.item.name,
          details: { catalog_only: true, sku: deleteTarget.item.sku ?? null },
        });
      } else {
        const res = await fetch(
          ...
          { method: "DELETE", headers: catalogApiHeaders({ userId, userEmail }) },
        );
        const json = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(typeof json.error === "string" ? json.error : "Failed to delete variant");
        await logWarehouseAction({
          action: WAREHOUSE_AUDIT_ACTIONS.DELETE_VARIANT,
          page: "/Warehouse_Backoffice/catalog/menu",
          method: "DELETE",
          entity_type: "variant",
          entity_id: deleteTarget.variant.id,
          entity_name: deleteTarget.variant.name,
          details: {
            catalog_only: true,
            item_id: deleteTarget.variant.item_id,
            sku: deleteTarget.variant.sku ?? null,
          },
        });
      }
      await load();
  ...
  const dispatchMiddlewareDelete = async () => {
    ...
    try {
      ...
      const selectedOutlets = middlewareOutlets
        .filter((outlet) => selectedOutletIds.includes(outlet.id))
        .map((outlet) => ({ id: outlet.id, name: outlet.name }));

      await logMiddlewareDispatch({
        page: "/Warehouse_Backoffice/catalog/menu",
        entity_type: deleteTarget.kind === "item" ? "product" : "variant",
        entity_id: deleteTarget.kind === "item" ? deleteTarget.item.id : deleteTarget.variant.id,
        entity_name: deleteTarget.kind === "item" ? deleteTarget.item.name : deleteTarget.variant.name,
        mode: "delete",
