...
import {
  loadSupabaseMiddlewareSalesCatalogIndex,
  resolveMiddlewareSaleCatalogLine,
} from "@/lib/middleware-sales-catalog-index";
import { fetchSupabasePosOrdersForOutlet } from "@/lib/supabase-pos-sales";
...
export async function handleOutletMiddlewareSalesRequestSupabase(
  request: NextRequest,
  ...
  try {
    ...
    const catalogIndex = await loadSupabaseMiddlewareSalesCatalogIndex(outletIds);
    const outletNames = new Map<string, string>();
    for (const outlet of await listSupabaseOutlets()) {
      outletNames.set(outlet.id, outlet.name);
    }

    const sales: SaleEvent[] = [];

    for (const currentOutletId of outletIds) {
      const orders = await fetchSupabasePosOrdersForOutlet({
      ...
      for (const order of orders) {
        const sourceEventId = order.source_event_id;
        ...
        const shiftId = order.shift_id ?? toNullableInt(shift?.shift_id);
    ...
    const message = error instanceof Error ? error.message : "Unable to load middleware sales from Supabase";
