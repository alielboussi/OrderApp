...
function mapOrderItems(
  ...
  return docs.map((docSnap) => {
    ...
    return {
      productId: (data.productId as string | null | undefined) ?? (data.product_id as string | null | undefined) ?? null,
      qty: Number(data.qty ?? 0),
    };
  });
}

export async function loadOutletCadenceOrderHistory(outletId: string): Promise<OutletCadenceOrderSnapshot[]> {
...
export async function assertOutletCadenceAllowsItems(
  outletId: string,
  items: Array<{ product_id?: string | null; productId?: string | null; qty: number; name?: string }>,
  ...
  const orders = await loadOutletCadenceOrderHistory(outletId);
  const violations = findOutletCadenceViolations(orders, cadenceItems, {
    excludeOrderId: options?.excludeOrderId,
  });

  if (violations.length === 0) return;
  throw new Error(buildOutletCadenceViolationMessage(violations) || "Outlet order limit reached.");
}
