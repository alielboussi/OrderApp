...
import styles from "./production-differences.module.css";
...
export default function WarehouseProductionDifferences() {
    ...
    const loadDifferences = async () => {
      ...
      try {
        ...
        const assignmentsRes = await supabase
          ...
          .select("finished_item_id,warehouse_id,variant_key,active")
          .eq("warehouse_id", warehouseId)
          .eq("active", true);
        ...
        const finishedIds = Array.from(
          new Set(
            ((assignmentsRes.data as Array<{ finished_item_id: string }> | null) ?? [])
              .map((row) => row.finished_item_id)
              .filter(Boolean)
          )
        );
        ...
        const [recipesRes, uomsRes, stockRes, entriesRes] = await Promise.all([
          supabase
            .from("recipes")
            .select(
          ...
          supabase
            ...
            .select("item_id,variant_key,qty_units,period_id")
            .eq("warehouse_id", warehouseId)
        ...
        const stockRows = (stockRes.data as WarehouseStockItem[] | null) ?? [];
        const entryRows = (entriesRes.data as ProductionEntry[] | null) ?? [];

        const conversionMap = buildConversionMap(uomRows);
        const stockByItem = new Map<string, number>();
        stockRows.forEach((row) => {
          if (normalizeVariantKey(row.variant_key) !== "base") return;
          stockByItem.set(row.item_id, toNumber(row.net_units, 0));
        });

        const producedByItem = new Map<string, number>();
        entryRows.forEach((row) => {
          if (normalizeVariantKey(row.variant_key) !== "base") return;
          const current = producedByItem.get(row.item_id) ?? 0;
          producedByItem.set(row.item_id, current + toNumber(row.qty_units, 0));
        });
        ...
        const rowsByFinished = new Map<string, ProductionRow>();
        ...
        recipes.forEach((recipe) => {
          ...
          const onHand = stockByItem.get(ingredientItem.id) ?? 0;
          const servings = Math.floor((onHand * yieldUnits) / componentQty);
          ...
          if (!existing) {
            const producedQty = producedByItem.get(recipe.finished_item_id) ?? 0;
          ...
          if (servings < existing.maxServings) {
            ...
            existing.bottleneckOnHand = onHand;
            existing.bottleneckNeededPerUnit = componentQty;
            existing.diffQty = existing.maxServings - existing.producedQty;
          }
        });
        ...
        if (!active) return;
        setRows([]);
        setError((err as Error).message ?? "Failed to load production differences.");
      } finally {
        if (active) setLoading(false);
      }
    };
      ...
      <main className={shellStyles.shell}>
        <header className={shellStyles.hero}>
          <div className={shellStyles.grow}>
            ...
            <h1 className={shellStyles.title}>Production Differences</h1>
            <p className={shellStyles.subtitle}>
              Compare recipe-based max servings against what was recorded as produced for the selected period.
            </p>
          </div>
        ...
        {!error && !loading && period && filteredRows.length === 0 && (
          <p className={styles.notice}>No production recipes found for this warehouse.</p>
        )}

        <section className={styles.tableCard}>
          <div className={styles.tableHeader}>
            <div>
              <h2 className={styles.tableTitle}>Production summary</h2>
              <p className={styles.tableMeta}>{filteredRows.length} finished items</p>
            </div>
          </div>
              ...
              <tbody>
                {filteredRows.flatMap((row) => {
                    ...
                    <tr key={row.key}>
                      ...
                      <td>
                        ...
                        <div className={styles.detailMuted}>
                          {row.bottleneckOnHand} on hand · {row.bottleneckNeededPerUnit} per unit
                        </div>
                      </td>
                    </tr>
                  );
                  ...
                  const detailRows = row.details.map((detail) => (
                    <tr key={`${row.key}-${detail.ingredientId}`} className={styles.detailRow}>
                      <td className={styles.detailText}>{detail.ingredientName}</td>
                      <td className={styles.detailText}>{detail.servings}</td>
                      <td className={styles.detailText}>{detail.onHand}</td>
                      <td className={styles.detailText}>--</td>
                      <td className={styles.detailText}>
                        <span className={styles.detailMuted}>Needs {detail.neededPerUnit} per unit</span>
                      </td>
                    </tr>
                  ));
