    ...
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      ...
      async function preloadVariations(productIds) {
        ...
        (data ?? []).forEach((variant) => {
          ...
          const seenVariantKeys = seenKeysByItem.get(variant.item_id) ?? new Set();
          if (seenVariantKeys.has(dedupeKey)) return;
          seenVariantKeys.add(dedupeKey);
          seenKeysByItem.set(variant.item_id, seenVariantKeys);
      ...
      function enforceOperatorLocks() {
        ...
        if (transferSubmit) {
          transferSubmit.disabled = state.loading || !consoleUnlocked || !transferDestinationSelected;
        }
        if (purchaseSubmit) {
