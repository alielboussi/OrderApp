...
const LOCKED_DEST_ID = DESTINATION_CHOICES[0]?.id ?? '';
const EXTRA_HOMES_IDS: string[] = [];
...
// IMPORTANT: Coldrooms storeroom scanner behavior mirrors the prior main warehouse scanner.
...
<body data-view="${initialView}" data-auth="true">
            ...
            <div class="locked-pill">
              ...
              <div class="destination-select-grid">
                <label class="destination-pill-select">
                  <span class="destination-pill-hint">Warehouses</span>
                  <button type="button" id="source-picker" class="destination-pill-button">Select warehouse</button>
  ...
  <div id="source-modal" class="select-modal" aria-hidden="true">
    <div class="select-modal-card" role="dialog" aria-modal="true" aria-labelledby="source-modal-title">
      <div class="select-modal-header" id="source-modal-title">Select source warehouse</div>
      ...
      <form id="purchase-form">
        <div class="purchase-shell">
          ...
          <div class="purchase-actions">
            <a id="purchase-back" class="button button-outline" href="/Coldrooms_Storeroom_Scanner" role="button">Back to Transfers</a>
      ...
      <form id="damage-form">
        <div class="damage-shell">
          ...
          <div class="damage-actions">
            <a id="damage-back" class="button button-outline" href="/Coldrooms_Storeroom_Scanner" role="button">Back to Transfers</a>
  ...
  <script>
    ...
    const SCANNER_ID = '1f9a3f3f-7407-4a9e-9d88-2ba249f88b13';
    ...
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      ...
      state.sourceSelection = lockedSourceId || null;
      ...
      function renderSourceOptions() {
        ...
        const placeholder = document.createElement('option');
        placeholder.value = '';
        placeholder.textContent = state.sourceOptions.length ? 'Select source' : 'No warehouses';
        sourceSelect.appendChild(placeholder);
        state.sourceOptions.forEach((option) => {
          if (!option?.id) return;
          const opt = document.createElement('option');
          opt.value = option.id;
          opt.textContent = option.label ?? 'Warehouse';
        ...
        if (sourcePicker) {
          sourcePicker.textContent = getSelectedSource()?.label ?? 'Select warehouse';
      ...
      function enforceOperatorLocks() {
        ...
        const sourceReady = Boolean(getSelectedSource());
      ...
      function getSourceOptionById(id) {
        ...
        const warehouseRecord = state.warehouses.find((w) => w.id === id);
        if (warehouseRecord) {
          return { id: warehouseRecord.id, label: warehouseRecord.name ?? 'Source warehouse' };
        }
        return null;
      }
      ...
      function getSelectedSource() {
        ...
        if (option) return option;
        return { id, label: 'Source warehouse' };
      }
      ...
      function ensureSourceSelected(shouldPrompt = true) {
        ...
        if (shouldPrompt) {
          showResult('Select a source warehouse before continuing.', true);
          showSourcePrompt();
        }
        return false;
      }
      ...
      async function handleSourceSelection(warehouseId) {
        ...
        if (!trimmed) {
          ...
          if (sourcePicker) {
            sourcePicker.textContent = 'Select warehouse';
          }
          setLockedWarehouseLabels(null, state.lockedDest, {
            sourceMissingText: 'Select warehouse',
        ...
        if (sourcePicker) {
          sourcePicker.textContent = option.label ?? 'Source warehouse';
        }
        setLockedWarehouseLabels(state.lockedSource, state.lockedDest, {
          sourceMissingText: 'Select warehouse',
          destMissingText: 'Select outlet'
        ...
        try {
          ...
          console.warn('Failed to refresh after source change', error);
          showResult('Unable to refresh source warehouse data.', true);
        }
      }
      ...
      async function refreshMetadata() {
        try {
          ...
          const hydratedSources = (SOURCE_CHOICES || []).map((choice) => {
            ...
            return {
              id: choice.id,
              label: record?.name ?? choice.label ?? 'Source warehouse'
            };
          });
          ...
          if (!hasSavedSource) {
            state.sourceSelection = hydratedSources[0]?.id ?? null;
          }
          lockedSourceId = state.sourceSelection ?? lockedSourceId;
          const sourceWarehouse = lockedSourceId
            ? state.warehouses.find((w) => w.id === lockedSourceId) ?? null
            : null;
          state.lockedSource = sourceWarehouse;
          if (sourceSelect) {
            sourceSelect.value = lockedSourceId || '';
          }
