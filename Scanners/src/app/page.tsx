    ...
    const base = q
      ? sorted.filter((t) => {
          const haystack = [
            ...
            t.items
              ?.map(
                (i) => `${i.product?.name ?? ""} ${i.variation?.name ?? ""} ${i.product_id ?? ""} ${i.variation_id ?? ""}`
              )
              .join(" ") ?? "",
          ]
            .join(" ")
      ...
      <main style={styles.shell}>
        <header style={styles.header}>
          ...
          <button
            style={{
              ...styles.iconBtn,
              ...(refreshSpinning ? styles.iconBtnSpin : null),
              ...(activeView === "backoffice" && !backofficeAvailable ? styles.iconBtnDisabled : null),
            }}
...
function LabeledDate(props: { label: string; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <span style={styles.label}>{label}</span>
      <input type="date" value={value} onChange={(e) => onChange(e.target.value)} style={styles.select} />
    </label>
  );
}
