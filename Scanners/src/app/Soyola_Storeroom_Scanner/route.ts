...
const LOCKED_SOURCE_ID = 'dd9e2d51-c631-4f71-85b0-d0e30cbfdae5';
...
const LOCKED_PRODUCT_ID = '2eebecc2-e42c-4c06-b2d2-46bd4736df0e';
...
// IMPORTANT: Soyola storeroom scanner behavior mirrors the prior main warehouse scanner.
...
<body data-view="${initialView}" data-auth="true">
  ...
  <script>
    ...
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      ...
      const supabase = supabaseClients.soyolaSession ?? getSupabaseClient(SESSION_STORAGE_KEY, {
        auth: {
      ...
      supabaseClients.soyolaSession = supabase;

      const passwordVerifier = supabaseClients.soyolaPassword ?? getSupabaseClient(PASSWORD_STORAGE_KEY, {
        auth: {
      ...
      supabaseClients.soyolaPassword = passwordVerifier;
      ...
      const LOCKED_PRODUCT_ID = ${JSON.stringify(LOCKED_PRODUCT_ID)};
      ...
      async function fetchProductsForWarehouse(warehouseIds) {
        ...
        try {
          ...
          if (LOCKED_PRODUCT_ID) {
            const lockedIds = new Set([LOCKED_PRODUCT_ID]);
            const lockedVariations = productsWithWarehouseVariations.has(LOCKED_PRODUCT_ID)
              ? new Set([LOCKED_PRODUCT_ID])
              : new Set();
      ...
      async function notifyTelegram(summary, context = 'transfer') {
        try {
          const response = await fetch('/api/notify-telegram', {
            ...
            body: JSON.stringify({ context, summary, scanner: 'soyola' })
          });
          if (!response.ok) {
            const info = await response.json().catch(() => ({}));
