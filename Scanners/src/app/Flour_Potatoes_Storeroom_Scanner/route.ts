...
const LOCKED_SOURCE_ID = '5662af7a-cf1d-4e6b-af50-c43f3be33312';
const HOMES_PARENT_ID = 'c021ea3b-7109-4690-be6c-0c7ae8278a5d';
const DESTINATION_CHOICES = [
  { id: '587fcdb9-c998-42d6-b88e-bbcd1a66b088', label: 'Select Outlet' }
] as const;
const LOCKED_DEST_ID = DESTINATION_CHOICES[0]?.id ?? '587fcdb9-c998-42d6-b88e-bbcd1a66b088';
const LOCKED_PRODUCT_IDS = [
  'ef65f24d-4eb4-4ed8-8d8f-a6bc0d9d72f9',
  'a1c3ab79-f764-4820-afeb-0f28c338cc90'
] as const;
...
// IMPORTANT: Flour Potatoes storeroom scanner behavior mirrors the prior main warehouse scanner.
...
<body data-view="${initialView}" data-auth="true">
      ...
      <form id="purchase-form">
        <div class="purchase-shell">
          ...
          <div class="purchase-actions">
            <a id="purchase-back" class="button button-outline" href="/Flour_Potatoes_Storeroom_Scanner" role="button">Back to Transfers</a>
      ...
      <form id="damage-form">
        <div class="damage-shell">
          ...
          <div class="damage-actions">
            <a id="damage-back" class="button button-outline" href="/Flour_Potatoes_Storeroom_Scanner" role="button">Back to Transfers</a>
  ...
  <script>
    ...
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      ...
      if (window.__flourPotatoesAppInitialized) {
        console.debug('Flour Potatoes scanner already initialized; skipping duplicate bootstrap');
        return;
      }
      window.__flourPotatoesAppInitialized = true;
      ...
      const supabase = supabaseClients.flourPotatoesSession ?? getSupabaseClient(SESSION_STORAGE_KEY, {
        auth: {
      ...
      supabaseClients.flourPotatoesSession = supabase;

      const passwordVerifier = supabaseClients.flourPotatoesPassword ?? getSupabaseClient(PASSWORD_STORAGE_KEY, {
        auth: {
      ...
      supabaseClients.flourPotatoesPassword = passwordVerifier;
      ...
      function openVariantModal(product, preferredVariation = null, context = state.mode) {
        ...
        rows.forEach((row) => {
          ...
          uom.textContent = formatUomDisplay(entry.baseUom);
      ...
      async function notifyTelegram(summary, context = 'transfer') {
        try {
          const response = await fetch('/api/notify-telegram', {
            ...
            body: JSON.stringify({ context, summary, scanner: 'flour-potatoes' })
          });
          if (!response.ok) {
            const info = await response.json().catch(() => ({}));
