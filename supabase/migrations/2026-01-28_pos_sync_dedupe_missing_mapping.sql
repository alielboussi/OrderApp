...
begin
  ...
  for v_item in select * from jsonb_array_elements(coalesce(payload->'items','[]'::jsonb)) loop
    ...
    if not found then
      v_missing_key := coalesce(v_item->>'pos_item_id','') || '|' || coalesce(v_item->>'flavour_id','');
      if not exists (
        select 1
        from public.pos_sync_failures
        where outlet_id = v_outlet
          and stage = 'missing_mapping'
          and details->>'pos_item_id' = v_item->>'pos_item_id'
          and coalesce(details->>'flavour_id','') = coalesce(v_item->>'flavour_id','')
      ) then
        insert into public.pos_sync_failures(
          outlet_id, source_event_id, pos_order_id, sale_id, stage, error_message, details
        ) values (
          v_outlet,
          v_source,
...
begin
  ...
  for v_item in select * from jsonb_array_elements(coalesce(payload->'items','[]'::jsonb)) loop
    ...
    if not found then
      v_missing_key := coalesce(v_item->>'pos_item_id','') || '|' || coalesce(v_item->>'flavour_id','');
      if not (v_missing_key = any(v_missing_keys)) then
        v_missing_keys := array_append(v_missing_keys, v_missing_key);
        v_errors := v_errors || jsonb_build_array(jsonb_build_object(
