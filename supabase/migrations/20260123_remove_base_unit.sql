-- Consolidate base_unit into consumption_unit and remove base_unit
...
BEGIN
  IF EXISTS (
    ...
    SET consumption_unit = COALESCE(NULLIF(consumption_unit::text, ''), base_unit::text, 'each')::qty_unit
    WHERE consumption_unit IS NULL OR trim(consumption_unit::text) = '';
  ELSE
    UPDATE public.catalog_items
    SET consumption_unit = COALESCE(NULLIF(consumption_unit::text, ''), 'each')::qty_unit
    WHERE consumption_unit IS NULL OR trim(consumption_unit::text) = '';
  END IF;
END $$;

ALTER TABLE public.catalog_items
ALTER COLUMN consumption_unit SET DEFAULT 'each';

-- Update record_outlet_sale functions to remove base_unit usage
