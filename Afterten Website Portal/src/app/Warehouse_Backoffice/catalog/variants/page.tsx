...
function CatalogVariantsPage() {
  ...
  const { status, readOnly, deleteDisabled, userId, userEmail } = useWarehouseAuth();
  ...
  const handleDeleteVariant = useCallback(
    async (variantId: string, itemId: string) => {
      ...
      try {
        ...
        await logWarehouseAction({
          ...
          entity_type: "variant",
          entity_id: variantId,
          entity_name: variant?.name ?? "Variant",
          details: { catalog_only: true, item_id: itemId, sku: variant?.sku ?? null },
        });
        await load();
      } catch (err) {
        console.error(err);
    ...
    [load, deleteDisabled, variants, userId, userEmail]
  );
