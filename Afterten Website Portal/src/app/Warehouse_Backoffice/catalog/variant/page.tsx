...
function VariantCreatePage() {
  ...
  const fetchNextVariantSku = useCallback(async () => {
    try {
      const res = await fetch("/api/catalog/next-pos-ids");
      if (!res.ok) return;
      const json = await res.json();
      if (typeof json.next_variant_sku === "string") {
        setForm((prev) => (prev.sku.trim() ? prev : { ...prev, sku: json.next_variant_sku }));
      }
    } catch (error) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const page = "/Warehouse_Backoffice/catalog/variant";
      ...
      await logWarehouseAction({
        ...
        details: { item_id: form.item_id, sku: form.sku || null },
      });

      const successMessage = editingId ? "Variant updated" : "Variant saved";
        ...
        <form className={styles.form} onSubmit={submit}>
          <div className={styles.fieldGrid}>
            ...
            <Field
              ...
              hint={
                editingId
                  ? "Single SKU for internal, supplier, and POS mapping"
                  : "Auto-filled with next POS variant SKU; leave blank to assign on save"
              }
              value={form.sku}
