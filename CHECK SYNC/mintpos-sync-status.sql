-- Till 1 MintPOS sync status (read-only)
-- Run in SSMS on the Till 1 PC against database MINTPOS.
--
-- Compare results with firestore-count.txt (run on your dev machine).
-- Fully synced when:
--   pending_bills = 0
--   firestore_bill_count = exportable_bills_with_lines
--   processed_bills    = exportable_bills_with_lines
...
-- Sample oldest pending bills (if any)
IF @pending > 0
BEGIN
  SELECT TOP 20
    ...
    s.time AS sale_time,
    s.uploadstatus,
    CONCAT('648e949d-8648-4c43-80d4-f08feb7bdd04-', bt.id) AS expected_firestore_doc_id
